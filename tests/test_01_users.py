...
class Test01UserAPI:
    ...
    def test_07_01_users_username_patch_admin(
        ...
        response = admin_client.patch(f"{self.USERS_URL}{user.username}/", data=data)
        assert response.status_code == HTTPStatus.OK, (
    ...
    def test_08_04_users_username_delete_superuser(
        ...
        users_cnt = django_user_model.objects.count()
        response = user_superuser_client.delete(f"{self.USERS_URL}{user.username}/")
        assert response.status_code == HTTPStatus.NO_CONTENT, (
    ...
    def test_10_02_users_me_has_field_validation(self, user_client, data, messege):
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST, messege[0].format(
            url=self.USERS_ME_URL, request_method=request_method
        )

    def test_10_03_users_me_patch_change_role_not_allowed(
        self, user_client, user, django_user_model
    ):
        response = user_client.patch(f"{self.USERS_ME_URL}", data=self.PATCH_DATA)
        assert response.status_code == HTTPStatus.OK, (
            "Проверьте, что PATCH-запрос пользователя с ролью `user` к "
            f"`{self.USERS_ME_URL}` возвращает ответ со статусом 200."
        )
